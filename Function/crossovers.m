...
if nargin>2
    ...
    % Interpolation method: 
    tmp = strncmpi(varargin,'SizeA',6);
    if any(tmp)
        SizeA = varargin{find(tmp)+1}; 
    end
end
...
%     [xi,yi,I,J] = intersections(x,y); 
     [xi,yi,I,J] = intersections(x(1:SizeA),y(1:SizeA),x(SizeA+1:end),y(SizeA+1:end)); 
end
...
xy2 = [x2 y2];
dxy1 = diff(xy1);   
dxy2 = diff(xy2);  %求矩阵中同一列上前后相邻元素之差
