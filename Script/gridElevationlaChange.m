...
% for ii=1:matSize-1
%     for jj=ii+1:matSize
%        path=strcat('.\Variate\',Region,'\CP\row_',num2str(ii,'%03d'),'\');   %CurrentPath is "..\Crossover"
%        ym1=string(ym1Mat(ii,jj));
%        ym2=string(ym2Mat(ii,jj));
%        Ascend=[ym1,ym2];
%        Descend=[ym2,ym1];
...
% E:\Sync\Master\Project\Crossover\Variate\Ross\CP\Row16
% E:\Sync\Master\Project\Crossover\Variate\Ross\CP\Row16

%% 删除一部分数据
% for ii=1:matSize-1
%     for jj=ii+1:matSize
%         if ym1Mat(ii,jj)>=201600||ym2Mat(ii,jj)>=201600
%             ym1=string(ym1Mat(ii,jj));
%             ym2=string(ym2Mat(ii,jj));
%             name_Total_CP=strcat(Region,'_',ym1,'_',ym2,'.mat'); 
%             path=strcat('E:\Sync\Master\Project\Crossover\Variate\Ross\CP\Row',string(ii),'\',name_Total_CP);
%             delete(path);
%         end
%     end
% end